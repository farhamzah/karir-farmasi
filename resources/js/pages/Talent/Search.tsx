...
export default function Search({ audience, heading, scope, filters, results }:{ audience:'company'|'internal'; heading:string; scope:string; filters:Filters; results:Card[] }) {
    ...
    return <><Head title="Talent Search"/><main className={'talent-page app-surface '+audience}>
        ...
        <section className="talent-layout"><aside className={drawer ? 'filter-panel open' : 'filter-panel'}><div><h2>Filter kandidat</h2><button className="filter-close" onClick={() => setDrawer(false)}>×</button></div>
            ...
            {results.length === 0 ? <EmptyState icon="⌕" title="Belum ada kandidat yang cocok" description="Ubah kata kunci atau filter. Kandidat hanya muncul setelah memberi izin."/> : results.map(r => <article className="talent-card" key={r.reference}><div className="talent-avatar">{r.professional_name.split(' ').map(x => x[0]).slice(0, 2).join('')}</div><div className="talent-card-body"><div className="talent-card-title"><div><h2>{r.professional_name}</h2><p>{r.headline || 'Profesional Farmasi'}</p></div>{r.open_to_work && <span className="open-badge">Terbuka</span>}</div><div className="talent-meta"><span>⌖ {r.city || 'Lokasi belum dicantumkan'}</span><span>⌁ {r.education || 'Pendidikan Farmasi'}</span></div><div className="talent-tags">{r.skills.map(x => <span key={x}>{x}</span>)}{r.badges.map(x => <span className="credential" key={x}>{x}</span>)}</div>{r.matched.length > 0 && <p className="match-reason"><strong>Matched:</strong> {r.matched.join(', ')}</p>}<div className="talent-card-actions"><Link className="primary-button" href={base+'/'+r.reference}>Lihat Profil Profesional</Link>{audience === 'company' && <button className="secondary-button" onClick={() => r.shortlisted ? router.delete(base+'/'+r.reference+'/shortlist') : router.post(base+'/'+r.reference+'/shortlist')}>{r.shortlisted ? 'Tersimpan' : 'Simpan Kandidat'}</button>}<button className="text-button" disabled title="Workflow kontak hadir pada tahap berikutnya">Minta Kontak · segera</button></div></div></article>)}
        </div></section>
    </main></>;
}
