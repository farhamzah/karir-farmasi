...
type Share = { url: string; allow_pdf_download: boolean; pdf_url: string | null; published_at: string | null };
...
export default function PublicCv({ cv, share, photoUrl }: { cv: CvPaperData; share: Share; photoUrl: string | null }) {
    ...
    const navigation = cv.sections.filter(section => section.key !== 'summary' && section.items.length > 0);
    const portfolioSections = cv.sections.filter(section => ['events','event_certificates'].includes(section.key));
    return <><Head title={`CV ${cv.professional_name}`}><meta name="robots" content="noindex,nofollow" /><meta name="referrer" content="no-referrer" /></Head><main className={`public-cv-page${cv.template.key === 'cv-02' ? ' public-cv-template-02' : ''}`}><header className="public-utility print-hidden"><div><span className="public-brand-mark">SK</span><span><strong>SAFA KARIR</strong><small>CV Alumni Farmasi UBP</small></span></div><nav>{share.allow_pdf_download && share.pdf_url && <a className="utility-primary" href={share.pdf_url}>Unduh PDF</a>}<button onClick={() => window.print()}>Cetak</button><button onClick={copy}>Salin tautan</button><button onClick={nativeShare}>Bagikan</button></nav></header>
        <div className="public-reading-head print-hidden"><div><p className="eyebrow">CURRICULUM VITAE · VERSI PUBLIK</p><h1>{cv.professional_name}</h1><p>Informasi pilihan alumni pada CV yang diterbitkan{share.published_at ? ` · ${new Date(share.published_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}` : ''}.</p></div><div className="social-share"><span>Bagikan CV</span><a href={`https://wa.me/?text=${title}%20${encoded}`} rel="noreferrer">WhatsApp ↗</a><a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encoded}`} rel="noreferrer">LinkedIn ↗</a><a href={`mailto:?subject=${title}&body=${encoded}`}>Email ↗</a></div></div>
