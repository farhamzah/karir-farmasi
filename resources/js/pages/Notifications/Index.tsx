...
import { companyNav } from '../../components/RoleNavigation';
...
export default function Index({ notifications, home, audience }:{ notifications:Item[]; home:string; audience:'candidate'|'company_user' }) {
    ...
    const nav = audience === 'candidate' ? candidateNav('notifications') : companyNav('inbox');
    return <><Head title="Inbox"/><main className="app-surface notification-page">
        <AppHeader context="INBOX KARIER" home={home} nav={nav} logout logoutUrl={audience === 'candidate' ? '/internal/session' : '/company/session'}/>
