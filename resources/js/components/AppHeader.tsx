...
export default function AppHeader({ context, home = '/dashboard', nav = [], actions, logout = false, logoutUrl = '/internal/session', verified = true }: {
    context: string;
    home?: string;
    nav?: NavItem[];
    actions?: ReactNode;
    ...
    return <header className="app-header">
        ...
        <div className="app-header-actions">{verified && <span className="app-trust-mark"><i aria-hidden="true" />Akses terverifikasi</span>}{roleContext?.can_switch && <Link className="app-role-switch" href="/pilih-role">Ganti ruang</Link>}{actions}{logout && <button className="ghost-button" onClick={() => router.delete(logoutUrl)}>Keluar</button>}</div>
