...
class CvTemplateVisualContractTest extends TestCase
{
    ...
    public function test_seven_templates_have_distinct_safe_visual_identities(): void
    {
        $templates = app(CvTemplateCatalog::class)->frontend();
        $this->assertSame(['cv-01', 'cv-02', 'cv-03', 'cv-04', 'cv-05', 'cv-06', 'cv-07'], array_column($templates, 'key'));
        $this->assertCount(7, collect($templates)->pluck('configuration')->map(fn ($config) => json_encode($config))->unique());
    ...
    public function test_switching_across_all_templates_and_back_preserves_cv_configuration(): void
    {
        ...
        foreach (['cv-02', 'cv-03', 'cv-04', 'cv-05', 'cv-06', 'cv-07', 'cv-01'] as $key) {
    ...
    public function test_all_templates_project_long_unicode_dense_and_empty_content_safely(): void
    {
        ...
        $session = ['core_principal' => $this->principal()];
        foreach (['cv-01', 'cv-02', 'cv-03', 'cv-04', 'cv-05', 'cv-06', 'cv-07'] as $key) {
