...
class CvTemplateVisualContractTest extends TestCase
{
    ...
    public function test_eight_templates_have_distinct_safe_visual_identities(): void
    {
        $templates = app(CvTemplateCatalog::class)->frontend();
        $this->assertSame(['cv-01', 'cv-02', 'cv-03', 'cv-04', 'cv-05', 'cv-06', 'cv-07', 'cv-08'], array_column($templates, 'key'));
        $this->assertCount(8, collect($templates)->pluck('configuration')->map(fn ($config) => json_encode($config))->unique());
    ...
    public function test_switching_across_all_templates_and_back_preserves_cv_configuration(): void
    {
        ...
        foreach (['cv-02', 'cv-03', 'cv-04', 'cv-05', 'cv-06', 'cv-07', 'cv-08', 'cv-01'] as $key) {
