...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $templateId = DB::table('cv_templates')->insertGetId([
            'key' => 'cv-06',
            'name' => 'Pharmacy Impact',
            'description' => 'CV A4 premium dengan header navy, timeline dua kolom, kompetensi, kegiatan, dan tautan profesional.',
            'active' => true,
            'category' => 'professional-impact',
            'base_template_key' => 'cv-06',
            'display_order' => 60,
            'created_at' => $now,
            'updated_at' => $now,
        ]);
    ...
    {
        $templateId = DB::table('cv_templates')->where('key', 'cv-06')->value('id');
        if ($templateId === null) {
            return;
        }
